# TP 3 : Génération Augmentée par Récupération (RAG)

**Master BDCC — SMA et IAD | Prof. RETAL SARA**

## Objectif pédagogique

Implémenter le pattern **RAG (Retrieval-Augmented Generation)** avec LangChain pour permettre à un agent LLM de répondre à des questions en s'appuyant sur des sources de données externes : un fichier PDF et une base de données SQL.
...
| Segmentation | `RecursiveCharacterTextSplitter` | Découpe en chunks (1000 caractères, chevauchement 200) |
...
**Base de données :** `Chinook.db` (base musicale — artistes, albums, pistes, etc.)
...
| Aspect | Partie 1 — RAG PDF | Partie 2 — SQL Agent |
|---|---|---|
| **Source de données** | Fichier PDF | Base SQLite |
| **Indexation** | Embeddings vectoriels | Pas d'indexation |
| **Recherche** | Similarité sémantique | Requêtes SQL exactes |
| **Rôle du LLM** | Synthèse du contexte récupéré | Génération de requêtes SQL |
