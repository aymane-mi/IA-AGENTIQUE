...
SYSTEM_PROMPT = (
    ...
    "(destinations, circuits, saisons, transports, culture, hebergement, budget et "
    "conseils pratiques). Utilise l'outil retrieve_documents pour t'appuyer sur la "
    "base documentaire avant de repondre a une question de connaissance. Utilise "
    "estimate_trip_budget pour calculer un budget de voyage, et "
    "suggest_destination_by_season pour recommander une destination selon la saison "
    "et le style de voyage. Reponds en francais clair, structure tes conseils, "
    "precise les limites si l'information manque, et cite les sources lorsque tu "
...
def grade_documents_node(state: AgentState) -> dict:
    ...
    prompt = (
        ...
        "question touristique ? Reponds uniquement par 'oui' ou 'non'."
    )
...
def rewrite_query_node(state: AgentState) -> dict:
    ...
    question = next((m.content for m in reversed(state["messages"]) if isinstance(m, HumanMessage)), "")
    prompt = (
        f"La question suivante n'a pas permis de trouver des documents touristiques pertinents : "
